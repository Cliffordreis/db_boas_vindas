    ...
    hbspt.forms.create({
        ...
        onFormReady: function($form) {
            ...
            styleTag.textContent = `
                .hbspt-form { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background: transparent; padding: 10px; }
                .hbspt-form .hs-form-field { margin-bottom: 15px !important; }
                .hbspt-form label { font-family: arial !important; display: block; margin-bottom: 5px !important; font-weight: 600 !important; color: #475569 !important; font-size: 13px !important; text-transform: uppercase !important; }
                .hbspt-form label.hs-error-msg {color:red !important;font-size:11px !important;font-weight:500 !important}
                .hbspt-form input[type="text"], .hbspt-form input[type="email"], .hbspt-form input[type="tel"] { width: 100%; padding: 12px 16px !important; border: 1.5px solid #e2e8f0; border-radius: 10px; background: #f8fafc; font-size: 15px; color: #2d3748; transition: all 0.3s ease; }
                .hbspt-form input:focus { outline: none; border-color: #1565C0; background: #ffffff; box-shadow: 0 0 0 3px rgba(21, 101, 192, 0.1); }
                .hbspt-form .hs-button.primary.large { width: 100%; padding: 16px; background: #1565C0; color: white; border: none; border-radius: 10px; font-size: 18px; font-weight: 700; text-transform: uppercase; cursor: pointer; margin-top: 15px; transition: all 0.3s ease; }
            ...
            if (nomeInput) {
                ...
                nomeInput.addEventListener('input', function(e) { this.value = this.value.replace(/[&=(){}[\]'"]/g, ''); this.setCustomValidity(''); this.reportValidity(); });
                nomeInput.addEventListener('blur', function() { validarNome(this); });
            }

            if (emailInput) {
                emailInput.setAttribute('required', 'required');
                emailInput.addEventListener('input', function(e) { this.value = this.value.replace(/[&=(){}[\]'"]/g, ''); this.setCustomValidity(''); this.reportValidity(); });
            ...
            if (corFundo) { submitButton.style.backgroundColor = '#'+corFundo; submitButton.style.backgroundImage = 'none'; }
            ...
            telefoneInput.addEventListener('keydown', function(e) {
                if (e.key === 'Backspace') {
                    ...
                    if (cursorPosition > 0 && (value[cursorPosition - 1] === ' ' || value[cursorPosition - 1] === '(' || value[cursorPosition - 1] === ')' || value[cursorPosition - 1] === '-')) {
                        e.preventDefault(); e.target.setSelectionRange(cursorPosition - 1, cursorPosition - 1);
                    }
                }
            });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ========================================================
    // MÁGICA AQUI: Puxa a URL do curso direto do HTML (FORMLINK)
    // ========================================================
    let urlDoCurso = '';
    const divConfig = document.querySelector('#iframeForm');
    
    if (divConfig) {
        const matchRegex = divConfig.innerHTML.match(/##FORMLINK=(.*?)##/);
        if (matchRegex && matchRegex[1]) {
            urlDoCurso = matchRegex[1].trim();
        }
    }

    // Se não encontrou URL no HTML, exibe erro e para por aqui.
    if (!urlDoCurso) {
        console.warn("URL do curso não encontrada na tag ##FORMLINK=...## do HTML.");
        ...
        if (elTitulo) elTitulo.textContent = "URL do curso não configurada";
        if (containerOpcoes) containerOpcoes.innerHTML = `<div class="!p-6 text-center text-red-500 !text-lg">Falta configurar a URL do curso no HTML da página.</div>`;
        return; 
    }

    // Usando a URL dinâmica extraída da própria página!
    ...
    // --- FUNÇÃO ATUALIZADA AQUI: Apenas a 1ª letra de todo o texto fica maiúscula ---
    ...
    fetch(urlProxy)
        ...
        .then(html => {
            try {
                ...
                // Duração (Lógica de Teste: Ignorando o SVG)
                let duracaoEncontrada = "Não informada";
                const spansIcone = doc.querySelectorAll('.prematricula-icon span');
                
                spansIcone.forEach(span => {
                    if (!span.querySelector('svg')) {
                        const texto = span.textContent.trim();
                        // Confere se tem 1 ou 2 digitos seguido de mes/mês/hora
                        if (/(\d{1,2})\s*(mes|mês|meses|hora|horas)/i.test(texto)) {
                            duracaoEncontrada = texto;
                        }
                    }
                });
                ...
                if(cardsDeOferta.length === 0){
                    ...
                    if (btnComprar) { btnComprar.innerText = 'Prosseguir para inscrição'; btnComprar.disabled = false; }
                    return;
                }
        ...
        .catch(err=>{
            ...
            if (elTitulo) { elTitulo.textContent = "Erro de conexão"; elTitulo.classList.remove('skeleton-text'); }
            if (elDuracao) { elDuracao.textContent = "Erro"; elDuracao.classList.remove('skeleton-text'); }
        });
});