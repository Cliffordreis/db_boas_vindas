...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Adicionamos a referência novamente para a Duração
    ...
    fetch(urlProxy)
        ...
        .then(html => {
            try {
                ...
                // Lógica Segura para Duração do Curso
                let duracaoEncontrada = "Não informada";
                const spansIcone = doc.querySelectorAll('.prematricula-icon span');
                
                spansIcone.forEach(span => {
                    // Confere se o span NÃO tem um elemento SVG dentro dele
                    if (!span.querySelector('svg')) {
                        const texto = span.textContent.trim();
                        // Busca por 1 ou 2 números acompanhados de mes/mês/meses ou hora/horas
